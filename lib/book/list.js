async function toc (book, req, reply) {
  ...
  if (['flat', 'nested'].includes(req.query.list)) req.session.bookList = req.query.list
  if (!req.session.bookList) req.session.bookList = 'flat'
  return reply.view('bajoWebBook:/book/toc', { book, pages, tree, list: req.session.bookList })
}
